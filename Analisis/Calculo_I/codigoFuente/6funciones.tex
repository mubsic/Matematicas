...
\tiny\tkzLabelXY[opacity=0.7,step=1, orig=false]
% label x, f(x)
\tkzDrawX[label=x,right=0.3]
\tkzDrawY[label=f(x),below = -0.6]
%dominio y función
\tkzFct[line width=1,domain = 0:1]{1} 
\tkzText[above,opacity=0.7](0.5,1){\tiny $f(x)=1$}
\tkzFct[line width=1,domain = 1:2]{2} 
\tkzText[above,opacity=0.7](1.5,2){\tiny $f(x)=2$}
% intervalos
\tkzSetUpPoint[shape=circle, size = 4, color=black, fill=black]
\tkzDefPointByFct[draw,with = a](0)
\tkzDefPointByFct[draw,with = a](1)
\tkzDefPointByFct[draw,with = b](2)
\tkzSetUpPoint[shape=circle, size = 4, color=black, fill=white]
\tkzDefPointByFct[draw,with = b](1)
\end{tikzpicture}
\end{center}

\item Poner $h(x) = f(2x).$ Describir el dominio de $g$ y dibujar su gráfica.
...
\tkzDrawY[label=f(x),below = -0.6]
%dominio y función
\tkzFct[line width=1,domain = -2:3]{\x} 
\tkzText[above,opacity=0.7](3,3){\tiny $f(x)=x$}
\tkzFct[line width=1,domain = -2:3]{\x**3} 
\tkzText[above,opacity=0.7](1,3){\tiny $f(x)=2x^3$}
\end{tikzpicture}
\end{center}
\end{ej}